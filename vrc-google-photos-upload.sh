...
main() {
    ...
    # Share the album so that people can see it, but not add to it or comment
    share=$(sendApiData "albums/$albumId:share" "{'sharedAlbumOptions':{'isCollaborative':'false','isCommentable':'false'}}")
	...
	for file in $files; do
		...
		mimeType=$(file -i "$file" | rev | cut -d' ' -f2 | rev)
		mimeType=${mimeType::-1}
		if [ "$mimeType" != "video/mp4" ] && [ "$mimeType" != "image/png" ] && [ "$mimeType" != "image/jpg" ] && [ "$mimeType" != "image/jpeg" ]; then
			continue
		fi
		...
		if [ "$recentlyAddedCheck" = false ]; then
			echo "First image uploaded. Perform these steps manually now to ensure proper order and labeling: Edit Album > Sort Photos > Recently Added, turn off automatic highlights"
	...
	printf "\nDon't forget to set the album cover image, and photo dates.\n"
}
